...
usage() {
  cat <<'EOF'
Usage: scripts/install/build_from_source.sh [options]

Builds the patched EvCode host from source and assembles the standard channel.
...
if [[ $SKIP_DEPS -ne 1 ]]; then
  "$SCRIPT_DIR/check_build_deps.sh" --channel standard
fi
...
cat <<EOF
EvCode standard source build complete.
host: $HOST_OUTPUT
dist: $DIST_OUTPUT_ROOT/standard

Try:
  $DIST_OUTPUT_ROOT/standard/bin/evcode --help
